...
function ItemIcon({ itemId, color }: { itemId: number; color: string }) {
  return (
    <div
      className="w-10 h-10 rounded-lg border-2 flex-shrink-0 overflow-hidden relative"
      style={{ borderColor: color }}
    >
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img
        src={`/api/icon?type=item&id=${itemId}`}
        alt=""
      ...
      />
      <div
        className="absolute inset-0 -z-10 flex items-center justify-center text-xs font-bold"
...
function ItemRow({ item, slotType }: { item: EquippedItem | undefined; slotType: string }) {
  ...
  return (
    <div className="flex items-center gap-3 py-1.5 px-2 rounded-lg hover:bg-gray-800/40 transition-colors group">
      {item ? <ItemIcon itemId={item.item?.id} color={color} /> : <EmptySlotIcon />}
      ...
      <div className="min-w-0 flex-1">
        ...
        {item ? (
          <>
            <div className="text-sm font-medium truncate leading-tight" style={{ color }}>
              {item.name}
            </div>
            ...
            {item.gems && item.gems.length > 0 && !item.enchantments?.length && (
              <div className="text-xs text-purple-400 leading-tight">
                {item.gems.length} Juwel{item.gems.length > 1 ? "en" : ""}
              </div>
            )}
